...
def update_metadata_with_csv(metadata, csv_path):
    ...
    # Add start and end times to the metadata
    metadata["confused_intervals"] = confused_times
    metadata["rh_intervals"] = rh_times

    # Convert intervals to frame intervals
...
def label_frames(body_tracking_data, intervals, label_key, average_frame_rate=30.0):
    ...
    for start, end in intervals:
        ...
        intervals_in_frames.append((start_frame, end_frame))
    
    for frame_number_str, frame_data in body_tracking_data.items():
        frame_number = int(frame_number_str)
        # Check if the frame number falls within any of the intervals
            ...
            if isinstance(inner_data, dict):
                ...
                for key, value in inner_data.items():
                    if isinstance(value, list):
                        ...
                        record[key] = value
                record["confused"] = bool(frame_data["confused"])
                record["help"] = bool(frame_data["help"])
...
def main(folder_path):
    ...
    # Load body tracking data
    body_tracking_data = read_json(os.path.join(folder_path, 'body_tracking.json'))

    # Label confused and RH frames
    for label_key, intervals_key in [("confused", "confused_intervals"), ("help", "rh_intervals")]:
        ...
        print("Labeling frames for", label_key)
    ...
    # Add frame-based intervals to metadata
    ...
    print("Saved body tracking data to", csv_output_path)
